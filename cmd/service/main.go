...
func init() {
	...
	rootCmd.Flags().String("olric-advertise-addr", store.DefaultAdvertiseAddr, "Olric advertise address (for NAT traversal, defaults to bind address)")
	rootCmd.Flags().Int("olric-advertise-port", store.DefaultAdvertisePort, "Olric advertise port (for NAT traversal and multi-instance testing, defaults to bind port)")
	rootCmd.Flags().Int("olric-memberlist-bind-port", store.DefaultMemberlistBindPort, "Olric memberlist bind port (0 for random, for multi-instance testing)")
	...
	_ = viper.BindPFlag("olric.advertise_addr", rootCmd.Flags().Lookup("olric-advertise-addr"))
	_ = viper.BindPFlag("olric.advertise_port", rootCmd.Flags().Lookup("olric-advertise-port"))
	_ = viper.BindPFlag("olric.memberlist_bind_port", rootCmd.Flags().Lookup("olric-memberlist-bind-port"))
